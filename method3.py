...
cat_list = ['Videos_&_DVDs','Books','Music','Video_Games','Toys','Online_Stores_&_Services','Software','Destinations','Cars']
ofd=open("Sample_op_S3_1_fold","w")
...
for i in range(len(rating_df)):
    category= Dict_Product_Category[rating_df['Product_id'][i]]
    user=rating_df['User_id'][i]
    List_Category_row.append(category)
    Dict_User_Category[user].add(category)
    user_rating_dict[user]['ratings'][i] = False
...
for loop_counter in range(k_fold_iteration):
    if loop_counter > 0:
        for userid in List_Users:
    ...
    # ofd.write("# ---------------------------------- initializing training matrices ------------------------------\n")
    ...
    # Filling Matrix S as Category_Circle (Trust Spliting)
    ...
    print "********************************"
    print "#--------------- initializing trainging parameters ----------------------------------"
    # ofd.write("#--------------- initializing trainging parameters ----------------------------------"+"\n")
    ...
    for category in Category_Circle.keys():
        ...
        for itr in range(iterations): 
            ...
            print itr,": Test round: ",loop_counter," , ",' RMSE_train : ', str(RMSE_train)
            # print "P[0]: ", P[0]
            # print "Q[0]: ", Q[0]
            # ofd.write("RMSE_train : "+str(RMSE_train)+"\n")
...
for cat in cat_error_val.keys():
    x=cat_error_val[cat]/float(k_fold_iteration)
